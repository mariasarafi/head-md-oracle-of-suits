...
function draw() {
  ...
  if (showMouthLandmarks && trackUserMouth) {
    ...
    if (calibratingSmile) {
      ...
      // call handlers if detected (handlers already invoked where appropriate)
      if (detectedInteraction === 'kiss') {
        if (typeof onKiss === 'function') try { onKiss(); } catch (e) {}
      } else if (detectedInteraction === 'smile') {
        if (typeof onSmile === 'function') try { onSmile(); } catch (e) {}
      } else if (detectedInteraction === 'sadness') {
        if (typeof onSadness === 'function') try { onSadness(); } catch (e) {}
      } else if (detectedInteraction === 'anger') {
        if (typeof onAnger === 'function') try { onAnger(); } catch (e) {}
      }
    }
  }
}
...
// Draw the season message next to the rotating suit (same style/positioning as displaySuitIntroMessage)
function drawSeasonEntryMessage() {
  ...
  if (deckIndex == null || !Array.isArray(DECKS) || !DECKS[deckIndex]) return;

  // Position calculation same as displaySuitIntroMessage
  const centerX = width / 2;
  const centerY = height / 2;
  const circleRadius = height * CONFIG.circleRadiusRatio;
  const angleStep = TWO_PI / 4;
  ...
  // place message a bit further out from suit
  const messageDistance = circleRadius * 1.5;
  const messageX = centerX + cos(suitAngle) * messageDistance;
  const messageY = centerY + sin(suitAngle) * messageDistance;
